...
//select NDO_ANMONTE, NDO_NUMEREPORT, PER_NOPERSO, PER_NOPRENO from NAI_DOS, PERSONNE,CHEVAL where NAI_DOS.NDO_NUPERSONAIS = PERSONNE.PER_NUPERSO;
    request = new Request("select distinct NDO_ANMONTE, NDO_NUMEREPORT, PER_NOPERSO, PER_NOPRENO from NAI_DOS, PERSONNE,CHEVAL where NAI_DOS.NDO_NUPERSONAIS = PERSONNE.PER_NUPERSO;", function(err) {  
    if (err) {  
    //    console.log(err);
    }  
    });    
